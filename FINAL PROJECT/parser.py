...
"""
    ...
    statement = <print> expression | if_statement | assignment_statement | include expression
...
def test_parse_factor():
    ...
    tokens = tokenize("-(6*7)")
    ast, tokens = parse_factor(tokens)
    assert ast == {'tag': 'negate', 'value': {'tag': '*', 'left': {'tag': 'number', 'value': 6}, 'right': {'tag': 'number', 'value': 7}}}
...
def test_parse_term():
    ...
    #this is my extra extended case
    tokens = tokenize("2/4*9")
    ast, tokens = parse_term(tokens)
    assert ast == {
        "tag": "*",
        "left": {
...
def parse_include_statement(tokens):
    assert tokens[0]["tag"] == "include"
    tokens = tokens[1:]

    if tokens[0]["tag"] in ["}", ";", None]:
        return {"tag": "include", "value": None}, tokens

    assert tokens[0]["tag"] == "identifier" 
    value = {"tag": "string", "value": tokens[0]["value"] + ".t"} #add our file extension
    return {"tag": "include", "value": value}, tokens[1:]
...
def parse_statement(tokens):
    ...
    if tag == "include":
        return parse_include_statement(tokens)
...
def test_parse_statement():
    ...
    assert (
        parse_statement(tokenize("include filename"))[0]
        == parse_include_statement(tokenize("include filename"))[0]
    )
...
def test_parse_include():
    """include statment = 'include' + identifier"""
    print("testing parse include")
    ast, tokens = parse_program(tokenize("include filename"))
    print(ast)
